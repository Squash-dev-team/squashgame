...
export const CHARACTERS = [
  { id: 'c_default', name: 'Rookie', rarity: 'Common', skin: '0x192e47', glow: '0x3ba3ff' },
  { id: 'c_crimson', name: 'Crimson Striker', rarity: 'Rare', skin: '0x4a1212', glow: '0xff4a40' },
  { id: 'c_neon', name: 'Neon Ninja', rarity: 'Rare', skin: '0x111a0d', glow: '0xc8ff3d' },
  { id: 'c_gold', name: 'Golden Ace', rarity: 'Rare', skin: '0x33250b', glow: '0xffb83b' },
  { id: 'c_cyber', name: 'Cyber Punk', rarity: 'Super Rare', skin: '0x0f2930', glow: '0x00f0ff' },
  { id: 'c_ghost', name: 'Ghost', rarity: 'Super Rare', skin: '0x404040', glow: '0xffffff' },
  { id: 'c_toxic', name: 'Toxic Hazard', rarity: 'Super Rare', skin: '0x2a123d', glow: '0x8a2be2' },
  { id: 'c_magma', name: 'Magma Core', rarity: 'Epic', skin: '0x3d1502', glow: '0xff771c' },
  { id: 'c_void', name: 'Void Walker', rarity: 'Epic', skin: '0x050505', glow: '0x9400d3' },
  { id: 'c_champ', name: 'Champion', rarity: 'Legendary', skin: '0xffd700', glow: '0xffffff' }
];
...
export function saveProfile() {
  try { localStorage.setItem(LOCAL_SAVE_KEY, JSON.stringify(playerProfile)); } catch (e) { /* storage may be unavailable */ }
}
...
function renderShop() {
  ...
  dom.rollBtn.disabled = playerProfile.coins < ROLL_COST;
  ...
  CHARACTERS.forEach(char => {
    ...
    card.innerHTML = `
      ...
      <div class="item-name">${char.name}</div><div class="item-stat">Cosmetic</div>
    `;
    ...
    else if (!isEquipped) btn.addEventListener('click', () => equipItem('char', char.id));
    card.appendChild(btn);
    dom.gridChar.appendChild(card);
  });
  ...
  RACQUETS.forEach(raq => {
    ...
    btn.textContent = isEquipped ? 'Equipped' : isUnlocked ? 'Equip' : '🔒 Locked';
    if (!isUnlocked) { btn.disabled = true; btn.style.opacity = '0.5'; btn.style.cursor = 'not-allowed'; }
    else if (!isEquipped) btn.addEventListener('click', () => equipItem('racquet', raq.id));
    card.appendChild(btn);
    dom.gridRacquet.appendChild(card);
  });
  ...
  CLASSES.forEach(cls => {
    ...
    btn.textContent = isEquipped ? 'Equipped' : isUnlocked ? 'Equip' : '🔒 Locked';
    if (!isUnlocked) { btn.disabled = true; btn.style.opacity = '0.5'; btn.style.cursor = 'not-allowed'; }
    else if (!isEquipped) btn.addEventListener('click', () => equipItem('class', cls.id));
    card.appendChild(btn);
    dom.gridClass.appendChild(card);
  });
}

function rollItem() {
  if (playerProfile.coins < ROLL_COST) return;
  playerProfile.coins -= ROLL_COST;

  const pool = [
    ...CHARACTERS.map(c => ({ ...c, itemType: 'char' })),
    ...RACQUETS.map(r => ({ ...r, itemType: 'racquet' })),
    ...CLASSES.map(c => ({ ...c, itemType: 'class' }))
  ];
  const pick = pool[Math.floor(Math.random() * pool.length)];
  const type = pick.itemType;
  ...
  if (unlockedList.includes(pick.id)) {
    ...
    dom.rollResult.innerHTML = `Duplicate <span class="rarity-label rarity-${rarityClass}" style="display:inline-block;">${pick.rarity}</span> ${pick.name} — converted to +50 🪙`;
  } else {
    unlockedList.push(pick.id);
    const typeName = type === 'char' ? 'Cosmetic' : type === 'racquet' ? 'Racquet' : 'Class';
    dom.rollResult.innerHTML = `🎉 New ${typeName}! <span class="rarity-label rarity-${rarityClass}" style="display:inline-block;">${pick.rarity}</span> ${pick.name}`;
  }
