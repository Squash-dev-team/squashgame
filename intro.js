...
export function initIntro(callbacks = {}) {
  ...
  function maybeShowTitle(t) {
    if (!titleShown && t >= TITLE_START) {
      ...
      // One broadcast-style shine sweep once the slam settles.
      setTimeout(() => mainTitleEl.classList.add('shine'), 550);
    }
  }
  ...
  function resetIntro() {
    ...
    mainTitleEl.classList.remove('slam', 'shine');
    flashOpacity = 0;
  ...
  // Skip straight to the title-slam + menu moment — no jarring cut, the
  // camera settles into the same hero shot the natural sequence ends on.
  function skipIntro() {
    if (buttonsShown) return;
    elapsed = Math.max(elapsed, BUTTONS_AT + 0.02);
    ball.visible = false;
