...
import { getProfile, CHARACTERS, RACQUETS, CLASSES } from './shop.js';

/* ---------------------------------------------------------
   EQUIPPED GEAR STATS
   Racquet + Class + Character multipliers, refreshed once per
   serve (resetForServe) so equipping something in the Pro Shop
   actually changes how the player moves/hits/tires out.
--------------------------------------------------------- */
let gearStats = { power: 1, speed: 1, aim: 1, stamina: 1, reach: 1 };

export function refreshGearStats() {
  const profile = getProfile();
  const char = CHARACTERS.find(c => c.id === profile.equipped.character) || CHARACTERS[0];
  const racquet = RACQUETS.find(r => r.id === profile.equipped.racquet) || RACQUETS[0];
  const cls = CLASSES.find(c => c.id === profile.equipped.class) || CLASSES[0];
  gearStats = {
    power: racquet.powerMult * cls.powerMult * char.powerMult,
    speed: racquet.speedMult * cls.speedMult * char.speedMult,
    aim: racquet.aimMult * char.aimMult,
    stamina: cls.stamMult * char.stamMult,
    reach: cls.reachMult
  };
}
...
export function attemptHit(who, forcedShotOverride, chargeRatio = 0) {
  ...
  if (ballState.status === 'serving') {
    ...
    const powerMult = who === 'player' ? (1 + chargeRatio * 0.8) * gearStats.power : 1;
  ...
  const reach = (who === 'player' && isDiving) ? (PLAYER_REACH * 1.5 * gearStats.reach) : (who === 'player' ? PLAYER_REACH * gearStats.reach : diff.reach);
  ...
  if (who === 'player') {
    ...
    else if (currentExecutedShot === 'lob') { ty = FRONT_H - 0.9; speed = 12.5 + Math.random() * 1.5; }
    else if (currentExecutedShot === 'kill') { ty = TIN_H + 0.08; speed = 12.0 * (1 + chargeRatio * 0.95); }
    else { ty = aimTarget.y; speed = 13.0 * (1 + chargeRatio * 0.95); }

    let errorSpread = (missRatio * missRatio * 2.4) / gearStats.aim;
    if (chargeRatio > 0.8) errorSpread *= 0.6;

    tx += (Math.random() - 0.5) * errorSpread;
    ty += (Math.random() - 0.5) * (errorSpread * 0.9);
    speed *= (1.0 - (missRatio * 0.2)) * gearStats.power;
    ...
    else { ty = (Math.random() < 0.28 ? 1.0 + Math.random() * 0.5 : 2.5 + Math.random() * 0.8); speed = 10.5 + diff.speed * 0.85; }
...
export function updatePlayer(dt) {
  ...
  if (isSprinting) playerStamina -= (22 * dt) / gearStats.stamina;
  else if (length > 0.8) playerStamina -= (8 * dt) / gearStats.stamina;
  else if (length < 0.5) playerStamina += 18 * dt * gearStats.stamina;
  ...
  if (isDiving) {
    ...
    currentMaxSpeed *= gearStats.speed;
