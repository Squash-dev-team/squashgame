...
io.on('connection', (socket) => {
  ...
  // Host-authoritative match state (rally outcomes, scores, serve
  // assignment). The room's player1 is the single source of truth for
  // scoring; this is a generic verbatim relay so the client owns the
  // payload shape instead of the server re-interpreting it.
  socket.on('matchState', (data) => {
    if (!currentRoomCode) return;
    socket.to(currentRoomCode).emit('opponentMatchState', data);
  });
