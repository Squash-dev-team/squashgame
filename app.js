...
function applyRemoteRally(payload) {
  if (!onlineMode || isHost || matchOver) return;
  ...
  if (payload.kind === 'let') {
    ...
    setTimeout(() => { whenResumed(() => startServe(flipSide(payload.server))); }, 2200);
    return;
  }
...
function endRally(winner, reason) {
  ...
  if (winner === 'let') {
    ...
    if (onlineMode) getSocket()?.emit('matchState', { kind: 'let', reason, server: ballState.server });
